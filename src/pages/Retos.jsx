...
export default function Retos() {
  ...
  useEffect(() => {
    if (decodedArea) {
      const completed = progressManager.getProgress().completedChallenges[decodedArea] || [];
  ...
  const handleCompleteChallenge = (challengeId, points) => {
    if (progressManager.completeChallenge(decodedArea, challengeId, points)) {
      ...
      alert("Ya completaste este reto anteriormente.");
    }
  };
      ...
      <div style={containerStyle}>
        ...
        <div style={headerStyle}>
          ...
          <p style={subtitleStyle}>
            ...
            Nivel actual: {progressManager.getProgress().level}
          </p>
        </div>
        ...
        <div style={statsBar}>
          <div>
            <strong>Progreso:</strong> {progressManager.getAreaStats(decodedArea).completed} de {areaChallenges.length} retos
          </div>
          <div style={progressBarContainer}>
            <div 
              style={{
                ...progressBarFill,
                width: `${progressManager.getAreaStats(decodedArea).percentage}%`
              }}
            ></div>
          </div>
        </div>
              ...
              >
                ...
                {activeChallenge?.id === challenge.id ? (
                  ...
                  <button 
                    ...
                    onClick={() => !isCompleted && setActiveChallenge(challenge)}
                    disabled={isCompleted}
                    aria-label={isCompleted ? `Reto completado: ${challenge.title}` : `Iniciar reto: ${challenge.title}`}
                  >
